...
--------------------- NEW!!!----------------------------
function ReaderProgress:genSummaryStats(width)
    ...
    local tile_height = height * (1/4)
    local user_duration_format = G_reader_settings:readSetting("duration_format")
    
    -- Calculate summary statistics
    local total_hours = self.total_time_in_sec and math.floor(self.total_time_in_sec / 3600) or 0
    local books_finished = self.finished_books or 0
    local books_read = self.total_books or 1
    local percent_finished = math.floor((books_finished / books_read) * 100)
    local avg_pages_per_min = self.average_pages_per_minute or 0
    local avg_min_per_session = self.average_minutes_per_session or 0
    local reading_streak = self.reading_streak_days or 0
    local current_book = self.current_book_title or _("Unknown")
    
    -- Row 1: Total hours and Books finished
    local row1_group = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextBoxWidget:new{
                alignment = "center",
                text = _("Total hours"),
                face = self.small_font_face,
                width = tile_width * 1.9,
            },
        },
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
    ...
    local row1_data = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextWidget:new{
                text = tostring(total_hours) .. " hrs",
                face = self.medium_font_face,
            },
        },
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextWidget:new{
                text = tostring(books_finished) .. " (" .. tostring(percent_finished) .. "%)",
                face = self.medium_font_face,
            },
        },
    }
    
    -- Row 2: Average pages/min and Average min/session
    local row2_group = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextBoxWidget:new{
                alignment = "center",
                text = _("Avg pages/min"),
                face = self.small_font_face,
                width = tile_width * 1.9,
            },
        },
    ...
    local row2_data = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextWidget:new{
                text = string.format("%.1f", avg_pages_per_min),
                face = self.medium_font_face,
            },
        },
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextWidget:new{
                text = string.format("%.0f min", avg_min_per_session),
                face = self.medium_font_face,
            },
        },
    }
    
    -- Row 3: Reading streak and Current book
    local row3_group = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextBoxWidget:new{
    ...
    local row3_data = HorizontalGroup:new{
        align = "center",
        CenterContainer:new{
            dimen = Geom:new{ w = tile_width * 2, h = tile_height },
            TextWidget:new{
                text = tostring(reading_streak) .. " days",
                face = self.medium_font_face,
            },
        },
        CenterContainer:new{
    ...
    local padding_span = HorizontalSpan:new{ width = Screen:scaleBySize(10) }
    ...
    table.insert(statistics_group, row1_group)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, row1_data)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, row2_group)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, row2_data)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, row3_group)
    table.insert(statistics_group, span_group)
    table.insert(statistics_group, row3_data)
